...
async function ensureUserRow(userId: string, email: string, name: string | null, image: string | null) {
  ...
  if (legacyUser) {
    // Migrate all app data from old id → new Clerk id
    ...
    // UserPlan will be regenerated by onboarding — delete the old one first
    await db.userPlan.deleteMany({ where: { userId: legacyUser.id } });
    // Rename old user's email so the unique constraint is freed
    await db.user.update({
      where: { id: legacyUser.id },
      data:  { email: `__migrated__${legacyUser.id}` },
    });
    // Create the new Clerk user row
    await db.user.create({ data: { id: userId, email, name, image } });
    // Delete the now-orphaned legacy row (all relations already moved)
